...
document.querySelectorAll('[data-save]').forEach((button) => {
  button.addEventListener('click', () => {
    ...
    if (section === 'gmail') {
      ...
      addLog(state.config.gmailFileName ? `Saved Gmail credentials placeholder: ${state.config.gmailFileName}` : 'No Gmail credentials file selected yet.');
    }

    if (section === 'openai') {
      state.config.openaiKey = openaiInput.value.trim();
      addLog(state.config.openaiKey ? 'Saved OpenAI key locally.' : 'OpenAI key cleared.');
    }

    if (section === 'sheets') {
      state.config.sheetId = sheetInput.value.trim();
      addLog(state.config.sheetId ? 'Saved Google Sheets ID locally.' : 'Google Sheets ID cleared.');
    }
...
runButton.addEventListener('click', async () => {
  ...
  const mockSteps = ['Fetched 5 emails', 'Summarized successfully', 'Tasks saved'];
  await wait(1100);

  mockSteps.forEach((message, index) => {
...
function renderLogs() {
  ...
  if (!state.logs.length) {
    logsList.innerHTML = '<div class="log-entry">No logs yet. Save settings or run the automation to begin.</div>';
    return;
  }
  ...
  state.logs.forEach((entry) => {
    ...
    row.innerHTML = `<strong>${entry.timestamp}</strong> — ${entry.message}`;
    logsList.appendChild(row);
  });
}

function renderConfigStatus() {
  const completed = [state.config.gmailFileName, state.config.openaiKey, state.config.sheetId].filter(Boolean).length;
